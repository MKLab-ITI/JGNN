	...
	protected void autosize(ArrayList<Tensor> lastInputs) {
		...
		if(input0 instanceof Matrix && input1 instanceof Matrix) {
			...
			if(getInputs().get(0) instanceof Parameter && left.getRows()==0 && left.getRowName().equals("?")) {
				...
				((Parameter)getInputs().get(0)).set(
						left.zeroCopy(right.getRows(), left.getCols())
							.setRowName(right.getRowName())
							.setColName(left.getColName())
							.setDimensionName(left.getDimensionName())
				).runPrediction();
				if(debugging)
			...
			if(getInputs().get(0) instanceof Parameter && left.getCols()==0 && left.getColName().equals("?")) {
				if(right.getCols()==0 && right.getColName().equals("?"))
					throw new RuntimeException("Cannot autosize based on two unknown dimensions");
				((Parameter)getInputs().get(0)).set(
						left.zeroCopy(left.getRows(), right.getCols())
							.setRowName(left.getRowName())
							.setColName(right.getColName())
							.setDimensionName(left.getDimensionName())
				).runPrediction();
				if(debugging)
					System.out.println("Automatically sized parameter: "+getInputs().get(0).describe());
			...
			if(getInputs().get(1) instanceof Parameter && right.getCols()==0 && right.getColName().equals("?")) {
				if(left.getCols()==0 && left.getColName().equals("?"))
					throw new RuntimeException("Cannot autosize based on two unknown dimensions");
				((Parameter)getInputs().get(1)).set(
						right.zeroCopy(right.getRows(), left.getCols())
							.setRowName(right.getRowName())
							.setColName(left.getColName())
							.setDimensionName(right.getDimensionName())
				).runPrediction();
				if(debugging)
			...
			if(getInputs().get(1) instanceof Parameter && right.getRows()==0 && right.getRowName().equals("?")) {
				if(left.getRows()==0 && left.getRowName().equals("?"))
					throw new RuntimeException("Cannot autosize based on two unknown dimensions");
				((Parameter)getInputs().get(1)).set(
						right.zeroCopy(left.getRows(), right.getCols())
							.setRowName(left.getRowName())
							.setColName(right.getColName())
							.setDimensionName(right.getDimensionName())
				).runPrediction();
			...
			if(getInputs().get(0) instanceof Parameter && matrix.getCols()==0 && matrix.getColName().equals("?")) {
				if(input1.size()==0 && input1.getDimensionName().equals("?"))
					throw new RuntimeException("Cannot autosize based on two unknown dimensions");
				((Parameter)getInputs().get(0)).set(
						matrix.zeroCopy(matrix.getRows(), input1.size())
							.setRowName(matrix.getRowName())
							.setColName(input1.getDimensionName())
							.setDimensionName(matrix.getDimensionName())
				).runPrediction();
				if(debugging)
					System.out.println("Automatically sized parameter: "+getInputs().get(0).describe());
			}
			if(getInputs().get(1) instanceof Parameter && input1.size()==0 && input1.getDimensionName().equals("?")) {
				if(matrix.getCols()==0 && matrix.getColName().equals("?"))
					throw new RuntimeException("Cannot autosize based on two unknown dimensions");
				((Parameter)getInputs().get(1)).set(
						input1.zeroCopy(matrix.getCols())
							.setDimensionName(matrix.getColName())
				).runPrediction();
				if(debugging)
					System.out.println("Automatically sized parameter: "+getInputs().get(1).describe());
			}
			...
			if(getInputs().get(0) instanceof Parameter && input0.size()==0 && input0.getDimensionName().equals("?")) {
				if(input1.size()==0 && input1.getDimensionName().equals("?"))
					throw new RuntimeException("Cannot autosize based on two unknown dimensions");
				((Parameter)getInputs().get(0)).set(
						input0.zeroCopy(input1.size())
							  .setDimensionName(input1.getDimensionName())
				).runPrediction();
				if(debugging)
					System.out.println("Automatically sized parameter: "+getInputs().get(0).describe());
			}
			if(getInputs().get(1) instanceof Parameter && input1.size()==0 && input1.getDimensionName().equals("?")) {
				if(input0.size()==0 && input0.getDimensionName().equals("?"))
					throw new RuntimeException("Cannot autosize based on two unknown dimensions");
				((Parameter)getInputs().get(1)).set(
						input1.zeroCopy(input0.size())
							  .setDimensionName(input0.getDimensionName())
				).runPrediction();
				if(debugging)
					System.out.println("Automatically sized parameter: "+getInputs().get(1).describe());
