...
public class LSTM {
	...
	public LSTMState output(Tensor input, LSTMState previousState) {
		 ...
		 * System.out.println("i "+i.describe()); System.out.println("f "+f.describe());
	...
	public Tensor updateTape(Tensor input, LSTMState previousState, Tensor outputErrorGradient) {
		...
		Tensor gradient_memory_tradeoff = Loss
				.sigmoidDerivative(f.multiply(previousMemory).selfAdd(i.multiply(memoryGate)))
				.selfMultiply(gradient_memory);
		...
		Tensor gradient_memoryGate_tradeoff = Loss
				.tanhDerivative(ug.transform(input).selfAdd(wg.transform(previousOutput)))
				.selfMultiply(gradient_memoryGate);
		...
		Tensor gradient_o_tradeoff = Loss.sigmoidDerivative(uo.transform(input).selfAdd(wo.transform(previousOutput)))
				.selfMultiply(gradient_o);
		...
		Tensor gradient_f_tradeoff = Loss.sigmoidDerivative(uf.transform(input).selfAdd(wf.transform(previousOutput)))
				.selfMultiply(gradient_f);
		...
		Tensor gradient_i_tradeoff = Loss.sigmoidDerivative(ui.transform(input).selfAdd(wi.transform(previousOutput)))
				.selfMultiply(gradient_i);
		 ...
		 * System.out.println("Gradient gradient_memory_tradeoff "
		 * +gradient_memory_tradeoff);
