...
/**
 * This class is a way to organize {@link NNOperation} trees into trainable
 * machine learning models. Critically, only model inputs and outputs need to be
 * defined. It also provides methods that perform training by calling forward
 * and backward passes. Models can have multiple inputs and outputs.
 * 
...
public class Model {
	 ...
	 * {@link ModelTraining#train(Model, Matrix, Matrix, Slice, Slice)} with the
	 * provided parameters.
	 * 
	 * @param trainer           The {@link ModelTraining} instance that is in charge
	 *                          of the training.
	 * @param features          A training feature {@link Matrix}, where each sample
	 *                          resides in one row.
	 * @param labels            A training label {@link Matrix} corresponding to
	 *                          features.
	 ...
	 * @return The model's instance (returns only after training concludes).
	 * @deprecated Training data have been moved inside ModelTraining instances to
	 *             allow a uniform interface for all training scheme automations.
	 *             Use {@link #train(ModelTraining)} instead.
	 */
	public Model train(ModelTraining trainer, Matrix features, Matrix labels, Slice trainingSamples,
			Slice validationSamples) {
		throw new RuntimeException("The method Model.train(ModelTraining, Matrix, Matrix, Slice, Slice) is depracated."
				+ "\n    Training data are now stored in ModelTraining instances."
				+ "\n    Use Model.train(ModelTraining) instead.");
	}

	/**
	 * Trains the model by calling {@link ModelTraining#train(Model)}.
	 * 
	 * @param trainer The {@link ModelTraining} instance that is in charge of the
	 *                training.
	 * @return The model's instance (returns only after training concludes).
	 */
	...
	/**
	 * Retrieves a list of operations by traversing the model's execution graph with
	 * the depth-first algorithm in the <b>inverse</b> edge order (starting from the
	 * outputs). This can be used by {@link Initializer} classes to push
	 * non-linearities to earlier layers.
	 * 
	...
	public ArrayList<Parameter> getParameters() {
		...
		while (!pending.isEmpty()) {
			NNOperation operation = pending.remove(pending.size() - 1);
			if (operation instanceof Parameter && !operation.isConstant() && !(operation instanceof Variable))
	...
	/**
	 * Toggles the training mode of the model. When in training mode, some outputs
	 * (e.g. those dependent on dropout) become stochastic.
	 * 
	...
	/**
	 * Retrieves a list of model outputs. Editing this list affects the model and is
	 * not recommended. Output order is based on the chronological addition of
	 * outputs through {@link #addOutput(NNOperation)}.
	 * 
	 * @return A list of {@link Variable} instances.
	 * @see #getInputs()
	 * @see #addOutput(NNOperation)
	...
	/**
	 * Forward run of the model given an array of input tensors. Wraps
	 * {@link #predict(List)}.
	 * 
	...
	/**
	 * Forward run of the model given a list of input tensors. Their order should
	 * match the order of variables in {@link #getInputs()}.
	 * 
	 * @param inputs A list of tensors to be assigned to input variables. These
	 *               should have
	 * @return A list of tensors output by the model after a forward pass.
	 * @see #predict(Tensor...)
	 * @throws IllegalArgumentException if the number of input tensors does not
	...
	public ArrayList<Tensor> predict(List<Tensor> inputs) {
		if (inputs.size() != this.inputs.size())
			throw new IllegalArgumentException(
					"Incompatible input size: expected" + this.inputs.size() + " inputs instead of " + inputs.size());
	 ...
	 *                       corresponding {@link #predict(List)} call. Element
	 *                       values should be either 1 or 0.
	 * @param desiredOutputs A list of output tensors that would be ideally returned
	 *                       by the prediction. Element values should lie in the
	 *                       rage [0,1].
	 * @return A list of prediction outputs (the ones computed before parameter
	 *         adjustment)
	 * @throws IllegalArgumentException If the number of inputs and desired outputs
	 *                                  do not match the sizes of
	 *                                  {@link #getInputs()} and
	 *                                  {@link #getOutputs()} respectively, or if
	 *                                  the number of weight tensor do not match the
	...
	/**
	 * Performs one parameter adjustment step (e.g. epoch) using {@link Optimizer}
	 * for a cross entropy loss function that compares desired outputs and the ones
	 * {@link #predict(List)} yields for the given inputs.
	 * 
	 * @param loss           The loss to minimize.
	 * @param optimizer      The provided optimizer with which to adjust values.
	 * @param inputs         A list of input tensors that would be passed to a
	 ...
	 *                       by the prediction. Element values should lie in the
	 *                       rage [0,1].
	 * @param weights        A list of weight tensors to be applied
	 *                       element-by-element on the outcome of
	...
	public List<Tensor> train(Loss loss, Optimizer optimizer, List<Tensor> inputs, List<Tensor> desiredOutputs,
			List<Tensor> weights) {
		if (inputs.size() != this.inputs.size())
			throw new IllegalArgumentException("Incompatible number of inputs: " + inputs.size() + " given but "
					+ this.inputs.size() + " expected");
		if (desiredOutputs.size() != this.outputs.size())
			throw new IllegalArgumentException("Incompatible number of outputs: " + desiredOutputs.size()
					+ " given but " + this.outputs.size() + " expected");
		setTraining(true);
		Memory.scope().enter();
		...
		Memory.scope().enter();
		for (int i = 0; i < outputs.size(); i++) {
			this.outputs.get(i).forceBackpropagate(optimizer, loss.derivative(outputs.get(i), desiredOutputs.get(i)));
		}
		setTraining(false);
		Memory.scope().exit();
		return outputs;
	 ...
	 * @param optimizer The provided optimizer with which to adjust values.
	 * @param inputs    A list of input tensors that would be passed to a
	 *                  corresponding {@link #predict(List)} call.
	 * @return The L2 loss (computed before parameter adjustment)
	 * @throws IllegalArgumentException If the number of inputs and outputs do not
	 *                                  match the sizes of {@link #getInputs()} and
	 *                                  {@link #getOutputs()} respectively.
	 * @see #train(Loss, Optimizer, List, List, List)
	 */
	public double trainTowardsZero(Optimizer optimizer, List<Tensor> inputs) {
		if (inputs.size() != this.inputs.size())
			throw new IllegalArgumentException(
					"Incompatible number of inputs: " + inputs.size() + " but " + this.inputs.size() + " expected");
		setTraining(true);
		ArrayList<Tensor> outputs = predict(inputs);
		double loss = 0;
		for (int i = 0; i < outputs.size(); i++) {
