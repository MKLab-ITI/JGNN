...
public class Dataset {
	...
	/**
	 * Retrieves a converter that maps samples to long identifiers that match them
	 * to rows of {@link #features()}, {@link #labels()}, and {@link #graph()}
	 * matrices. For example, a list of all node ids can be obtained per
	...
	/**
	 * Retrieves a converter that maps class names to label dimentions. For example,
	 * the prediction for one sample can be converted to its name per
	 * <code>dataset.classes().get(prediction.argmax())</code>.
	 * 
