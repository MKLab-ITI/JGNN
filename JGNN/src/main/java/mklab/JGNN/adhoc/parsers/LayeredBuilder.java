...
/**
 * Extends the capabilities of the {@link ModelBuilder} with the ability to
 * define multilayer (e.g. deep) neural architectures. The symbols
 * <code>{l}</code> and <code>{l+1}</code> are replaced in all expressions with
 * appropriate layer identifiers (these increase by one each time a new
...
public class LayeredBuilder extends ModelBuilder {
	...
	/**
	 * Instantiates a layered builder with the given symbol as an input name. If you
	 * plan to immediately use a {@link #layer(String)} expression that involves
	 * <code>X{l}</code>, where <code>X</code> is some symbol, set <code>X0</code>
	 * as the architecture's input. You can add more architecture inputs normally
	 * with {@link #var(String)}.
	 * 
	...
	/**
	 * Applies an {@link #operation(String)} and increases the layer identifier
	 * count.
	 * 
	...
	public LayeredBuilder layer(String expression) {
		expression = expression.replace("{l+1}", "" + (layer + 1)).replace("{l}", "" + layer);
	...
	/*
	 * public LayeredBuilder classify() { layer("h{l+1}=h{l}[nodes]");
	 * layer("h{l+1}=softmax(h{l}, row)"); out("h"+layer); return this; }
	 */
	/**
	 * Repeats a {@link #layer(String)} definition a number of times. Ideal for
	...
	public LayeredBuilder operation(String desc) {
		desc = desc.replace("{l+1}", "" + (layer + 1)).replace("{l}", "" + layer);
		for (String layerId : rememberAs.keySet())
	...
	/**
	 * Concatenates horizontally the output of a number of given layers, starting
	 * from the last one and going backwards. (For concatenation of specific layers
	 * just use <code>concat</code> within normal operations.)
	 * 
	...
	/**
	 * Defines a number of {@link #config(String, double)} symbols involving a
	 * <code>{l}</code> notation, for example so that they can be used during
	 * {@link #layerRepeat(String, int)}.
	 * 
	 * @param config The configuration symbols (these should involve
	 *               <code>{l}</code>).
	 * @param func   A lambda Java function to calculate the configuration's value.
	 *               This takes as input an integer (starting from 0 for the current
	 *               layer) and adds one for each subsequently declared symbol.
	...
	/**
	 * Defines a number of {@link #constant(String, double)} symbols involving a
	 * <code>{l}<code>
	 ...
	 * @param func         A lambda Java function to calculate the constant's value.
	 *                     This takes as input an integer (starting from 0 for the
	 *                     current layer) and adds one for each subsequently
	 *                     declared symbol.
